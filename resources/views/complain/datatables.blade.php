...
<html lang="en">
							...
							<div class="container">
								...
								<div class="card-header flex-wrap py-5">
										<div class="card-title">
											<h3 class="card-label">Complaints Datatable
										</div>
										<div>
											<!-- Modal--></div>
											...
											<form action="{{ route('ticket.change', 1 )}}">
											@csrf
											<div class="form-group row">
											<div class="col-lg-12 pb-3">
											<label>Staff</label>
											<select class="form-control  form-control-solid @error('flat_no') is-invalid @enderror" name="name" id="kt_select2_1"> 
											<option>Select Staff</option>
											@foreach($user as $users)
											@if($users->type=='Staff')
											<option>{{ $users->name}} {{ $users->ltname}}</option>
											@endif
											@endforeach
											@error('flat_no')
  ...
  <tr>
	  ...
	  <td>{{ $rent->subject }}</td>
	  <td>
	  @if( $rent->is_active==1 && $rent->is_assigned==1)
		<span class="label label-lg font-weight-bold label-light-success label-inline">Active</span>
		@elseif( $rent->is_active==1 && $rent->is_assigned==0)
		<span class="label label-lg font-weight-bold label-light-warning label-inline">Assigned</span>
		@else
